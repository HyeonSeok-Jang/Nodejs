...
/**
 * express: 모듈을 실행해 app에 할당. 서버 내부에 http 모듈이 내장되어있어 서버 역할을 할 수 있음
 * coockie-parser: 쿠키를 해석해 req.cookies 객체로 생성 해당 객체의
 *                  비밀키는 .env파일에 저장되어 있음
 * morgan: 서버와 연결 후, 요청 메소드/응답 상태 코드를 터미널에서 볼 수 있도록 함
 * path: 입력받은 String값을 경로로 구분자'/'를 이용해 반환해주는 패키지
 * express-session: 로그인 등의 이유로 세션을 구현하거나 사용자 데이터를 임시 저장할때 사용
